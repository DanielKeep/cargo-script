...
impl<'s> Manifest<'s> {
    pub fn into_toml(self) -> Result<toml::Table> {
        use self::Manifest::*;
        match self {
            Toml(s) => Ok(toml::Parser::new(s).parse().ok_or("could not parse embedded manifest")?),
            TomlOwned(ref s) => Ok(toml::Parser::new(s).parse().ok_or("could not parse embedded manifest")?),
    ...
    fn dep_list_to_toml(s: &str) -> Result<toml::Table> {
        ...
        Ok(toml::Parser::new(&r).parse().ok_or("could not parse embedded manifest")?)
    }
}
